...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterInput) => {
    setIsLoading(true);
    const success = await registerUser(data.name, data.username, data.email, data.password, data.dni, selectedBackground.url);
    if (success) {
      router.push('/');
    }
    setIsLoading(false);
          ...
          <Card>
            <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>Debe contener 8 dígitos, sin puntos ni letras.</p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
              ...
              <CardFooter className="flex flex-col gap-4">
                <Button type="submit" className="w-full" disabled={isLoading}>{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Registrarme</Button>
