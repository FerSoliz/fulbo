...
const PREDEFINED_COLORS = ['#000000', '#FFFFFF', '#FF0000', '#0000FF', '#008000', '#FFFF00', '#FFA500', '#800080', '#808080', '#A52A2A'];

const productSchema = z.object({
  name: z.string().min(3, { message: 'El nombre debe tener al menos 3 caracteres.' }),
  tienda: z.string().min(1, { message: 'El nombre de la tienda es obligatorio.' }),
...
export function ProductEditDialog({ isOpen, onOpenChange, onSave, productToEdit, currentUser }: ProductEditDialogProps) {
...
function ProductForm({ onSave, onFinished, productToEdit, currentUser }: ProductFormProps) {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 max-h-[70vh] overflow-y-auto pr-4">
                ...
                <FormField control={form.control} name="sizes" render={({ field }) => (<FormItem><FormLabel>Talles Disponibles</FormLabel><FormControl><Input placeholder="S, M, L, XL" {...field} /></FormControl><FormDescription>Escribe los talles separados por comas.</FormDescription><FormMessage /></FormItem>)} />
                ...
                <FormField control={form.control} name="imageUrl" render={({ field }) => (<FormItem><FormLabel>Imagen</FormLabel><FormControl><Input type="file" onChange={(e) => e.target.files && handleImageUpload(e.target.files[0])} /></FormControl><FormMessage /></FormItem>)} />
