  ...
  (
    ...
    if (collapsible === "none" && !isMobile) {
      return (
        <div
          className={cn(
            "flex h-full w-[var(--sidebar-width)] flex-col bg-card text-card-foreground border-r",
            className
          )}
          ref={ref}
          {...props}
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-1 top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-foreground transition-transform hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 peer-hover/menu-button:text-accent-foreground [&>svg]:size-4 [&>svg]:shrink-0",
        // Increases the hit area of the button on mobile.
        "after:absolute after:-inset-2 after:md:hidden",
